...
fn immediate_and_inspector_overridden_calls_preserve_parent_metadata() {
    ...
    impl Inspector<test_utils::TestContext> for MockCall {
        fn call(
            &mut self,
            _: &mut test_utils::TestContext,
            input: &mut CallInputs,
        ) -> Option<CallOutcome> {
            if !self.intercept || input.target_address != self.target {
                return None;
            }
            ...
            Some(CallOutcome::new(
                InterpreterResult::new(
                    ...
                    Bytes::new(),
                    Gas::new(input.gas_limit),
                ),
                input.return_memory_offset.clone(),
            ))
        }
    }

    for intercept in [false, true] {
        let origin = Address::repeat_byte(0x31);
        let outer = Address::repeat_byte(0x32);
        let inner = Address::repeat_byte(0x33);
        let immediate = Address::repeat_byte(0x34);
        let arb_sys = address!("0000000000000000000000000000000000000064");
        let mut context = setup_context();
        context
            .arb_state(None, false)
            ...
            .unwrap();
        fund_account(&mut context, origin, U256::from(100_000_000_u64));
        let mut code = call_forwarder(immediate, false).original_bytes().to_vec();
        code.truncate(code.len() - 7);
        code.extend_from_slice(&call_forwarder(arb_sys, false).original_bytes());
        install(&mut context, inner, Bytecode::new_raw(code.into()));
        install(&mut context, outer, call_forwarder(inner, false));
        ...
        let result = evm
            .inspect_one_tx(
                TxEnv {
                    ...
                    data: Bytes::copy_from_slice(
                        &keccak256("myCallersAddressWithoutAliasing()")[..4],
                    ),
                    ..Default::default()
                }
                .into(),
            ...
            .unwrap();
        let ExecutionResult::Success { output, .. } = result else {
            panic!("nested call failed: {result:?}");
        };
        assert_eq!(Address::from_slice(&output.data()[12..32]), outer);
        assert_eq!(evm.0.inspector.intercepted, intercept);
    }
}
...
fn reverted_redeem_does_not_publish_scheduled_retry() {
    assert_reverted_redeem(false);
}

#[test]
fn caught_reverted_redeem_does_not_publish_scheduled_retry() {
    assert_reverted_redeem(true);
}

fn assert_reverted_redeem(catch_revert: bool) {
    ...
    let target = if catch_revert {
        let outer = Address::repeat_byte(0x56);
        install(&mut context, outer, call_forwarder(helper, false));
        outer
    } else {
        helper
    };
    ...
    if catch_revert {
        assert!(result.is_success(), "outer call failed: {result:?}");
    } else {
