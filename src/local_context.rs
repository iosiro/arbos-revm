...
pub trait ArbitrumLocalContextTr: LocalContextTr {
    ...
    fn exit_frame(&mut self, success: bool);
...
pub struct ArbitrumLocalContext {
    ...
    /// Retry queue lengths at entry to each live frame, for journal rollback.
    pub frame_retry_checkpoints: Vec<usize>,
...
impl Default for ArbitrumLocalContext {
    fn default() -> Self {
        Self {
            ...
            frame_retry_checkpoints: Vec::new(),
...
impl LocalContextTr for ArbitrumLocalContext {
    fn clear(&mut self) {
        ...
        self.frame_retry_checkpoints.clear();
...
impl ArbitrumLocalContextTr for ArbitrumLocalContext {
    ...
    fn enter_frame(&mut self, caller: Address) {
        ...
        self.frame_retry_checkpoints
            .push(self.scheduled_retries_on_commit.len());
    }

    fn exit_frame(&mut self, success: bool) {
        self.frame_callers.pop();
        if let Some(checkpoint) = self.frame_retry_checkpoints.pop()
            && !success
        {
            self.scheduled_retries_on_commit.truncate(checkpoint);
        }
    }
